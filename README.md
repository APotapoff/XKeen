...
> *Перед обновлением Xray для обеспечения корректной работы XKeen необходимо удалить файл **02_transport.json** из директории **opkg\etc\xray\con***REMOVED***gs**. Обратите внимание, что в новой версии этот файл больше не используется, поэтому возвращать его не нужно. После этого добавьте в верхнюю часть файла **05_routing.json** следующую строку:*
> ```
...
> *В случае появления ошибки <mark>«panic: runtime error: slice bounds out of range»</mark> или неожиданных сбоев в работе Xray, рекомендуется выполнить [обновление ядра Xray](https://github.com/Corvus-Malus/XKeen?tab=readme-ov-***REMOVED***le#обновление-ядра-xray-до-последней-версии) на роутере. Данная мера часто позволяет устранить ошибки и восстановить корректную работу системы.*
